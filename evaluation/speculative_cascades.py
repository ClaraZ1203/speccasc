...
def _speculative_cascades(
    ...
    if deferral is None:
        # Specified per paper
        tv_distance = torch.clamp(p_t - q_t, 0).sum(dim=-1).unsqueeze(-1)
        ...
        selected_tokens_mask = q_t < p_t.max(dim=-1).values.unsqueeze(-1) * (1 - alpha)
        eta = torch.sum(selected_tokens_mask * q_t, dim=-1).unsqueeze(-1)
        pi = (~selected_tokens_mask) * q_t + eta * p_t
...
if __name__ == "__main__":
    ...
    sample_input = tokenizer("Who is Buzz Aldrin?", return_tensors='pt')
    t0 = time()
    outputs = target_model.generate(
        **sample_input,
        assistant_model=draft_model,
        custom_generate=functools.partial(_assisted_decoding, assistant_model=draft_model, alpha=0.1), 
        do_sample=True,
    )
    t1 = time()
